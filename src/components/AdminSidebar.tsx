    ...
    <aside className="w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col fixed h-full hidden md:flex">
      <div className="p-6 flex flex-col h-full">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-slate-900 dark:text-white text-xs font-black leading-tight tracking-tighter">Gestión de Méritos</h1>
            <p className="text-slate-500 dark:text-slate-400 text-xs font-bold tracking-widest truncate max-w-[120px]">{schoolName}</p>
          </div>
        </div>
        ...
        <nav className="flex flex-col gap-2 flex-1">
          {menuItems.map((item) => (
            <Button 
              key={item.path}
              variant="light" 
              className={`justify-start gap-3 px-3 h-11 font-bold text-xs tracking-widest ${location.pathname === item.path ? 'bg-[#1e3b8a]/10 text-[#1e3b8a]' : 'text-slate-500'}`} 
          ...
          <Button 
            variant="light" 
            className={`justify-start gap-3 px-3 h-11 font-bold text-xs tracking-widest ${location.pathname === '/admin/configuracion' ? 'bg-[#1e3b8a]/10 text-[#1e3b8a]' : 'text-slate-500'}`} 
        ...
        <div className="mt-auto pt-6 border-t border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-3 p-2 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
            ...
            <div className="flex-1 min-w-0 cursor-pointer" onClick={onOpen}>
              <p className="text-xs font-black truncate text-slate-900 dark:text-white tracking-tighter">{adminData?.name}</p>
              <p className="text-xs text-slate-400 truncate font-black tracking-widest">{adminData?.role}</p>
            </div>
            <Button 
              isIconOnly 
              variant="light" 
