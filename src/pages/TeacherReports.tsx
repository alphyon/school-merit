      ...
      {isLoading ? ( <div className="flex justify-center py-20"><Spinner size="lg" /></div> ) : (
        <div className="space-y-8 text-slate-900">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <Card className="border-none shadow-sm bg-white p-6 flex flex-row items-center gap-4"><div className="p-4 bg-blue-50 text-blue-600 rounded-2xl font-black uppercase text-[10px]">Matrícula</div><div><p className="text-2xl font-black">{stats.totalStudents}</p></div></Card>
             <Card className="border-none shadow-sm bg-white p-6 flex flex-row items-center gap-4"><div className="p-4 bg-red-50 text-red-600 rounded-2xl font-black uppercase text-[10px]">Deméritos Anual</div><div><p className="text-2xl font-black">{stats.demerits}</p></div></Card>
          </div>
          <Card className="bg-white border-none shadow-sm overflow-hidden"><CardBody className="p-0 text-slate-900"><table className="w-full text-xs text-left"><thead className="bg-gray-50 font-bold text-gray-400 uppercase text-[10px]"><tr><th className="p-4">Mes</th><th className="p-4 text-center">Faltas</th><th className="p-4 text-center">Redenciones</th></tr></thead><tbody>{monthlyData.filter(m => m.monthIndex >= parseInt(startMonth) && m.monthIndex <= parseInt(endMonth)).map((m, i) => (<tr key={i} className="border-b border-gray-50"><td className="p-4 font-bold uppercase">{months[m.monthIndex-1].label}</td><td className="p-4 text-center text-red-600 font-bold">{m.demTotal}</td><td className="p-4 text-center text-emerald-600 font-bold">{m.redTotal}</td></tr>))}</tbody></table></CardBody></Card>
        </div>
      )}
