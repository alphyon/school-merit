    ...
    <DashboardLayout role="admin">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 text-slate-900">
        ...
        <div className="flex gap-2 items-center bg-white p-2 rounded-xl shadow-sm border border-gray-50">
          ...
          <Select aria-label="Y" size="sm" className="w-24" selectedKeys={[dirYear]} onSelectionChange={(k) => setDirYear(Array.from(k)[0] as string)}>{["2024", "2025", "2026"].map((y) => <SelectItem key={y}>{y}</SelectItem>)}</Select>
          <Button isIconOnly color="primary" className="bg-[#1e3b8a]" onPress={() => printDirector()}><Printer size={18} /></Button>
        </div>
      </div>
        ...
        <div>
          <Card className="border-none shadow-sm bg-[#1e3b8a] text-white h-full">
            <CardBody className="p-8 space-y-6">
              <h3 className="text-lg font-black uppercase flex items-center gap-2"><FileText size={24} /> Reporte Grado</h3>
              <div className="grid grid-cols-2 gap-2">
                <Select aria-label="S" label="Desde" variant="faded" classNames={{ trigger: "text-white" }} selectedKeys={[teachStart]} onSelectionChange={(k) => setTeachStart(Array.from(k)[0] as string)}>{monthsList.map((m) => <SelectItem key={m.key}>{m.label}</SelectItem>)}</Select>
                <Select aria-label="E" label="Hasta" variant="faded" classNames={{ trigger: "text-white" }} selectedKeys={[teachEnd]} onSelectionChange={(k) => setTeachEnd(Array.from(k)[0] as string)}>{monthsList.map((m) => <SelectItem key={m.key}>{m.label}</SelectItem>)}</Select>
              </div>
